...
async def lock(event, perm):
    ...
    if "text" in input_str:
       await dion.edit_permissions(event.chat_id, send_messages=False)
       await event.reply("Locked `text`.")
    elif "media" in input_str:
       await dion.edit_permissions(event.chat_id, send_media=False)
       await event.reply("Locked `media`.")
    elif "sticker" in input_str:
       await dion.edit_permissions(event.chat_id, send_stickers=False)
       await event.reply("Locked `sticker`.")
    elif "gifs" in input_str:
       await dion.edit_permissions(event.chat_id, send_gifs=False)
       await event.reply("Locked `gifs`.")
    elif "forward" in input_str:
       await dion.edit_permissions(event.chat_id, forwards=False)
       await event.reply("Locked `forward`.")
    elif "games" in input_str:
       await dion.edit_permissions(event.chat_id, send_games=False)
       await event.reply("Locked `games`.")
    elif "inline" in input_str:
       await dion.edit_permissions(event.chat_id, send_inline=False)
       await event.reply("Locked `inline`.")
    elif "polls" in input_str:
       await dion.edit_permissions(event.chat_id, send_polls=False)
       await event.reply("Locked `polls`.")
    elif "preview" in input_str:
       await dion.edit_permissions(event.chat_id, embed_link_previews=False)
       await event.reply("Locked `preview`.")
    elif "all" in input_str:
       await dion.edit_permissions(event.chat_id,
          send_messages=False, 
          send_media=False,
          send_stickers=False,
          send_gifs=False,
...
async def unlock(event, perm):
    ...
    if not input_str:
       ...
       return
    if "text" in input_str:
       await dion.edit_permissions(event.chat_id, send_messages=True)
       await event.reply("Unlocked `text`.")
    elif "media" in input_str:
       await dion.edit_permissions(event.chat_id, send_media=True)
       await event.reply("Unlocked `media`.")
    elif "sticker" in input_str:
       await dion.edit_permissions(event.chat_id, send_stickers=True)
       await event.reply("Unlocked `sticker`.")
    elif "gifs" in input_str:
       await dion.edit_permissions(event.chat_id, send_gifs=True)
       await event.reply("Unlocked `gifs`.")
    elif "forward" in input_str:
       await dion.edit_permissions(event.chat_id, forwards=True)
       await event.reply("Unlocked `forward`.")
    elif "games" in input_str:
       await dion.edit_permissions(event.chat_id, send_games=True)
       await event.reply("Unlocked `games`.")
    elif "inline" in input_str:
       await dion.edit_permissions(event.chat_id, send_inline=True)
       await event.reply("Unlocked `inline`.")
    elif "polls" in input_str:
       await dion.edit_permissions(event.chat_id, send_polls=True)
       await event.reply("Unlocked `polls`.")
    elif "preview" in input_str:
       await dion.edit_permissions(event.chat_id, embed_link_previews=True)
...
@dion.on(events.callbackquery.CallbackQuery(data="locks"))
async def _(event):
